...
# Submit official HLE full-split answers unchanged to the native Gym HLE verifier.
# Usage: bash scripts/run_oracles/hle_vision.sh [--prepare-only | positive-task-limit]
# Requires HF_TOKEN for preparation and INFERENCE_API_KEY for the judge.
# Results: results/hle_vision_oracle.jsonl; logs: results/hle_vision_oracle/.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/../.."
source scripts/run_oracles/oracle_ports.sh
oracle_ports hle_vision
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    echo 'Usage: bash scripts/run_oracles/hle_vision.sh [--prepare-only | positive-task-limit]'
    echo 'Prepare official full-split HLE answers and score them with the native Gym HLE judge.'
    echo 'Requires HF_TOKEN and INFERENCE_API_KEY. Results/logs: results/hle_vision_oracle*.'
    exit 0
fi
PREPARE_ONLY=false
...
LOG_DIR="results/hle_vision_oracle"
OUTPUT_JSONL="results/hle_vision_oracle.jsonl"
mkdir -p "$LOG_DIR"
echo "==> Preparing benchmark data..."
uv run --locked python -c 'from benchmarks.hle.prepare_vision import prepare; prepare()' \
...
set -m
uv run --locked gym env start --config scripts/run_oracles/configs/hle_vision.yaml \
...
uv run --locked python scripts/run_oracles/submit_references.py \
    ...
    +benchmark_jsonl=benchmarks/hle/data/hle_benchmark_vision.jsonl \
    +resources_server=hle_vision_equivalence_llm_judge_resources_server \
