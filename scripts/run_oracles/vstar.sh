...
# Published VStarBench answers -> native Gym verifier -> per-row results.
# Usage: bash scripts/run_oracles/vstar.sh [--prepare-only] [positive-limit]
set -euo pipefail
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    echo 'Usage: bash scripts/run_oracles/vstar.sh [--prepare-only] [positive-limit]'
    exit 0
fi
PREPARE_ONLY=false
...
mkdir -p results/vstar_oracle
echo '==> Preparing benchmark data...'
uv run --locked python benchmarks/vstar/prepare.py 2>&1 | tee results/vstar_oracle/prepare.log
...
echo '==> Starting Gym...'
set -m
RAY_ENABLE_UV_RUN_RUNTIME_ENV=0 uv run --locked gym env start --config benchmarks/vstar/config.yaml \
    "${ORACLE_GYM_START_PORT_ARGS[@]}" \
    ++vstar_simple_agent=null ++vlm_eval_kit_simple_agent=null \
    > results/vstar_oracle/server.log 2>&1 &
GYM_ENV_PID=$!
set +m
cleanup() {
...
for i in $(seq 1 180); do
    if ! kill -0 "$GYM_ENV_PID" 2>/dev/null; then
        echo 'Gym exited; inspect results/vstar_oracle/server.log' >&2
        exit 1
    fi
...
uv run --locked python scripts/run_oracles/submit_references.py \
    ...
    +benchmark_jsonl=benchmarks/vstar/data/vstar_benchmark.jsonl \
    +resources_server=vstar_resources_server +reference_field=answer \
    +output_jsonl=results/vstar_oracle.jsonl "${LIMIT_ARGS[@]}" \
    2>&1 | tee results/vstar_oracle/oracle.log
echo '==> Results: results/vstar_oracle.jsonl'
